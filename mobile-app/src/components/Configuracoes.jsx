...
function Configuracoes({
  ...
  async function importar(event) {
    ...
    event.target.value = "";
    if (!arquivo) return;
    const confirmar = window.confirm("Importar este banco substituirá todos os usuários, notas, categorias e pendências atuais. Um backup de segurança será criado antes. Continuar?");
    if (!confirmar) return;
    const resultado = await executar(
      () => importarBancoService(arquivo),
      (dados) => `${dados.mensagem} Backup de segurança: ${dados.backupSeguranca}.`
    );
    if (resultado?.reiniciando) setTimeout(() => onBancoImportado?.(), 900);
  }
    ...
    <section className="space-y-5 animate-fade-in">
      <article className={`rounded-[2rem] border p-5 md:p-7 shadow-2xl ${painel}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-6">
          <div className={`rounded-2xl border p-4 ${temaEscuro ? "bg-slate-950 border-slate-800" : "bg-slate-50 border-slate-200"}`}><span className="text-xs text-slate-400">Nome</span><strong className="block mt-1">{usuario?.nome}</strong></div>
          <div className={`rounded-2xl border p-4 ${temaEscuro ? "bg-slate-950 border-slate-800" : "bg-slate-50 border-slate-200"}`}><span className="text-xs text-slate-400">Usuário</span><strong className="block mt-1">@{usuario?.usuario}</strong></div>
          <div className={`rounded-2xl border p-4 ${temaEscuro ? "bg-slate-950 border-slate-800" : "bg-slate-50 border-slate-200"}`}><span className="text-xs text-slate-400">Permissão</span><strong className="block mt-1">{usuario?.admin ? "Administrador" : "Usuário"}</strong></div>
        </div>
      ...
      {secao === "usuarios" && usuario?.admin ? (
        ...
        <AdminSubcategorias temaEscuro={temaEscuro} categorias={categorias} subcategorias={subcategorias} criarSubcategoria={criarSubcategoria} editarSubcategoria={editarSubcategoria} excluirSubcategoria={excluirSubcategoria} />
      ) : (
        <>
          <article className={`rounded-[2rem] border p-5 md:p-7 ${painel}`}>
            <h3 className="text-xl font-black">Privacidade das notas</h3>
            <p className="text-sm text-slate-400 mt-3 leading-relaxed">Notas privadas aparecem somente para o autor e administradores. Notas públicas aparecem na Comunidade para os demais usuários. Também é possível proteger uma nota pública com senha.</p>
          </article>

          {usuario?.admin && (
            <article className={`rounded-[2rem] border p-5 md:p-7 ${painel}`}>
              <h3 className="text-xl font-black">Backup e banco de dados</h3>
              <p className="text-sm text-slate-400 mt-3">Os backups automáticos continuam ativos internamente. O backup mobile inclui usuários, notas, categorias, pendências, checklists e demais dados locais.</p>
              ...
              <div className="mt-5 divide-y divide-emerald-500/15">
                {backups.slice(0, 8).map((item) => <div key={item.nome} className="py-3 flex items-center justify-between gap-3 text-sm"><span className="truncate">{item.nome}</span><span className="shrink-0 text-slate-400">{Math.max(1, Math.round(item.tamanho / 1024))} KB</span></div>)}
